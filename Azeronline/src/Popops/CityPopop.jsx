    ...
    <>
      ...
      <div className="popopCity">
        <div
          ...
          style={
            ScroolItemData || categoryItem || ArxivItem || XidmetlerItem || fiberoptikapopop || SertlerItemData
              ? { width: "80%" }
              : { width: "50%" }
          }
        >
          ...
          {(ScroolItemData || categoryItem || ArxivePopop || XidmetlerItem || fiberoptikapopop || SertlerItemData) && (
            <div className="Scool-container">
              ...
              <div className="ScroolCategory">
                {
                ScroolItemData && ScroolItemData.title.map((item, id) => (
                    <p key={id}>{item.text}</p>
                  ))
                  }
                {
                categoryItem && categoryItem.title.map((item, id) => (
                    <p key={id}>{item.text}</p>
                  ))}
                {
                !ArxivItem && Arxivedata.map((item, id) => (
                ...
                )}
                {
                  XidmetlerItem && (
                    <div className="XidmetlerItem-Popop">
                       {
                        XidmetlerItem.title && XidmetlerItem.title.map((item,id)=>(
                          <p key={id}>
                            {
                            (item.text && item.text ) || 
                            (item.h4 && (
                                    ...
                                    <tr key={id}>
                                      {
                                        ...
                                        <>
                                          ...
                                          {
                                            i.Kanalar && i.Kanalar.map((item,id)=>(
                                              <span key={id}>
                                                <img src={item.img} />
                                                <a className="doshed" href={item.link}>{item.text}</a>
                                              </span>
                                            ))
                                          }
                             ...
                             (
                              item.strong && item.strong.map((it,id)=>(
                                <>
                                   {it.text && (
                                    <span key={id}>{it.text}</span>
                                   )}
                                   {
                                    it.textLink && (
                                      <a className="doshed" href={it.link}>{it.textLink}</a>
                                    )
                                   }
                                </>
                              ))
                            ...
                            <p className="sertlerNavigate">
                              {
                                item.strong.map((item,id)=>(
                                  <span key={id}>
                                    {(item.text && item.text)|| <a href={item.link}> { item.textLink && item.textLink}</a>}
                                  </span>
                                ))
                              }
                            </p>
